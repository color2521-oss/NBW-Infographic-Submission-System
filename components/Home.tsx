...
export const Home: React.FC<HomeProps> = ({ isAdmin }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setShowForm(false);
      setNewTitle(''); setNewContent(''); setNewImage(null);
  ...
  const showFullImage = (url: string) => {
    ...
    Swal.fire({
      html: `<div style="padding: 10px;"><img src="${formattedUrl}" referrerpolicy="no-referrer" style="width: 100%; max-height: 80vh; object-fit: contain; border-radius: 12px;" /></div>`,
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 space-y-6">
      <div className="text-center py-10 bg-white rounded-3xl shadow-sm border border-gray-100 relative overflow-hidden">
        ...
        {isAdmin && !showForm && (
          <button onClick={() => setShowForm(true)} className="mt-6 bg-nbw-600 text-white px-6 py-2 rounded-full font-medium hover:bg-nbw-700 transition shadow-lg flex items-center gap-2 mx-auto">
      ...
      {isAdmin && showForm && (
        <div className="bg-white p-6 rounded-2xl shadow-xl border border-blue-100 animate-fade-in">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
             <input className="w-full border rounded-lg px-4 py-2 focus:ring-2 focus:ring-nbw-500 outline-none" placeholder="หัวข้อประกาศ" value={newTitle} onChange={e => setNewTitle(e.target.value)} required />
             <textarea className="w-full border rounded-lg px-4 py-2 focus:ring-2 focus:ring-nbw-500 outline-none h-32" placeholder="รายละเอียด..." value={newContent} onChange={e => setNewContent(e.target.value)} required />
      ...
      <div className="space-y-6">
        {!loading && announcements.map(ann => {
          const isPinned = DataService.parseBool(ann.isPinned);
          return (
            <div key={ann.id} className={`bg-white rounded-2xl shadow-md overflow-hidden hover:shadow-xl transition-all duration-300 flex flex-col md:flex-row group ${isPinned ? 'border-2 border-nbw-200 ring-2 ring-nbw-50' : ''}`}>
              {ann.imageUrl && (
                <div className="w-full md:w-56 h-48 md:h-auto flex-shrink-0 bg-gray-100 relative cursor-zoom-in overflow-hidden" onClick={() => showFullImage(ann.imageUrl!)}>
                  <img src={DataService.formatDriveUrl(ann.imageUrl)} referrerPolicy="no-referrer" alt={ann.title} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" />
              ...
              <div className="p-6 flex-grow flex flex-col justify-center min-h-[160px] relative">
                {isPinned && <div className="absolute top-4 right-4 text-nbw-500 animate-pulse"><Pin size={20} fill="currentColor" /></div>}
